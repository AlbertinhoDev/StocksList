...
class MainViewController: UIViewController {
    ...
    private func setupTags() {
        ButtonsForMain.allCases.forEach { button in
            tag.append(button.id)
        }
    ...
    private func showTableView1(animated: Bool) {
            if animated {
                UIView.animate(withDuration: 0.35, delay: 0, options: .curveEaseInOut, animations: {
                    self.stocksTableView.transform = .identity
                    self.favouriteTableView.transform = CGAffineTransform(translationX: self.view.bounds.width, y: 0)
                }, completion: { _ in
                    self.isTableView1Active = true
                })
            } else {
                stocksTableView.transform = .identity
                favouriteTableView.transform = CGAffineTransform(translationX: view.bounds.width, y: 0)
                isTableView1Active = true
            }
    //        switchButton.setTitle("Показать таблицу 2", for: .normal)
        }
        
        private func showTableView2(animated: Bool) {
            if animated {
                UIView.animate(withDuration: 0.35, delay: 0, options: .curveEaseInOut, animations: {
                    self.stocksTableView.transform = CGAffineTransform(translationX: -self.view.bounds.width, y: 0)
                    self.favouriteTableView.transform = .identity
                }, completion: { _ in
                    self.isTableView1Active = false
                })
            } else {
                stocksTableView.transform = CGAffineTransform(translationX: -view.bounds.width, y: 0)
                favouriteTableView.transform = .identity
                isTableView1Active = false
            }
    //        switchButton.setTitle("Показать таблицу 1", for: .normal)
        }
//    private func showTableView1(animated: Bool) {
